  ...
  return (
    <div role="dialog" aria-modal="true" className={cls("fixed inset-0 z-50 max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto flex flex-col", exiting ? "animate-slide-down" : "animate-slide-up", dark ? "bg-gray-900" : "bg-gray-50")}>
      <header className={cls("flex items-center justify-between p-4 border-b", dark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100")}>
        ...
        <button onClick={() => setComposing(true)} aria-label="새 글 쓰기" className="text-brand-500"><PenLine size={20}/></button>
      </header>
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {composing && (
          <div className={cls("p-5 rounded-card space-y-3", dark ? "bg-gray-800" : "bg-white")}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={cls("text-xs px-2 py-0.5 rounded-full font-bold", dark ? "bg-brand-900/40 text-brand-200" : "bg-brand-50 text-brand-700")}>Day {dayNum}</span>
            </div>
            <input value={postTitle} onChange={(e) => setPostTitle(e.target.value)} placeholder="제목 (선택)"
              className={cls("w-full min-h-tap px-4 py-3 rounded-btn text-[15px] font-semibold border outline-none focus:ring-2 focus:ring-brand-500/20",
                dark ? "bg-ink-900 text-ink-50 border-ink-700 focus:border-brand-500" : "bg-white text-ink-900 border-ink-200 focus:border-brand-500")}/>
            <textarea value={postBody} onChange={(e) => setPostBody(e.target.value)} placeholder="오늘의 챌린지를 공유해보세요..."
              rows={3}
              className={cls("w-full px-4 py-3 rounded-btn text-[15px] resize-none border outline-none focus:ring-2 focus:ring-brand-500/20 leading-[1.6]",
                dark ? "bg-ink-900 text-ink-50 border-ink-700 focus:border-brand-500" : "bg-white text-ink-900 border-ink-200 focus:border-brand-500")}/>
            <div className="flex gap-2">
              ...
              <button onClick={handlePost} disabled={!postBody.trim()}
                className={cls("flex-1 py-2.5 rounded-xl text-sm font-black",
                  postBody.trim() ? "bg-brand-500 text-white" : dark ? "bg-gray-700 text-gray-600" : "bg-gray-200 text-gray-400")}>
                공유하기
              </button>
            </div>
          </div>
        ...
        {sorted.length === 0 && !composing && (
          <div className={cls("py-12 text-center rounded-card border-2 border-dashed", dark ? "border-gray-700 text-gray-500" : "border-gray-200 text-gray-400")}>
        ...
        {sorted.map((p) => (
          <div key={p.id} className={cls("p-5 rounded-card", dark ? "bg-gray-800" : "bg-white")}>
            <div className="flex items-center gap-2 mb-2">
              <CircleUser size={16} className={p.anonId === anonId ? "text-brand-500" : dark ? "text-gray-400" : "text-gray-500"}/>
              ...
              {p.anonId === anonId && <span className={cls("text-xs px-1.5 py-0.5 rounded-full font-bold", dark ? "bg-brand-900/40 text-brand-200" : "bg-brand-50 text-brand-700")}>나</span>}
            </div>
